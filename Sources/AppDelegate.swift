...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let logger = Logger(subsystem: "com.ranax.prompthub", category: "AppDelegate")
    ...
    private func showStorageStartupFailure(_: Error) {
        let alert = NSAlert()
        alert.messageText = "PromptHub couldn't open its local storage"
        alert.informativeText = "Your prompt files were left untouched. Please restart PromptHub or contact support if this keeps happening."
    ...
    private func showRestartAlert() {
        let alert = NSAlert()
        alert.messageText = "PromptHub needs to restart"
    ...
    private func setupMenuBarIcon() {
        ...
        if let image = NSImage(systemSymbolName: "p.square", accessibilityDescription: "PromptHub")?
